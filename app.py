...
from compare import theme, data, portfolio
...
with tab_compare:
    ...
    if cat == "health":
        ...
        fig = px.line(
            ...
            title=f"เบี้ยประกันสุขภาพ ณ อายุ {age} ปี ตามระดับ Deductible",
        ...
        fig = px.bar(
            ...
            title=f"เบี้ยประกันชีวิต ณ อายุ {age} ปี (ทุนประกันอ้างอิงในตาราง)",
...
with tab_deductible:
    ...
    hdf_raw = data.health_df(age=age)
    ...
    with col2:
        st.metric("อายุที่ใช้คำนวณเบี้ย", f"{age} ปี ({data.age_band_label(age)})")
...
with tab_portfolio:
        ...
        with c3:
            ...
            if f_cat == "health":
                f_deductible = st.selectbox(
                    "Deductible เดิม (บาท)", [0, 30_000, 50_000, 100_000],
    ...
    if sim_cat == "health":
        sdf = data.health_df(age=age)
    elif sim_cat == "investment":
        sdf = data.investment_df()
    elif sim_cat == "life":
        sdf = data.life_df(age=age)
    else:
        sdf = data.retirement_df(age=age)
