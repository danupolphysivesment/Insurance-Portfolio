...
def health_df(age: int | None = None) -> pd.DataFrame:
    df = pd.DataFrame(HEALTH_PRODUCTS).copy()
    mult = age_multiplier(age) if age is not None else 1.0
    df["annual_premium"] = (df["annual_premium_base"] * mult).round(-2)
    return df


def investment_df() -> pd.DataFrame:
    return pd.DataFrame(INVESTMENT_PRODUCTS).copy()


def life_df(age: int | None = None) -> pd.DataFrame:
...
def retirement_df(age: int | None = None) -> pd.DataFrame:
